...
const cardRow = (dispatch, className, card) => {
  if (card.toggle === 0) {
...
const cardsBody = (dispatch, className, cards) => {
...
const tableView = (dispatch, cards) => {
  if (cards.length === 0) {
    ...
    return form(
      ...
      [
        div({ className: "flex gap-4" }, [
          fieldSet("Frontcard", description, "Enter Frontcard text...", e =>
            dispatch(frontInputMsg(e.target.value))
          ),
          fieldSet("Backcard", back || "", "Enter Backcard text...", e =>
            dispatch(backInputMsg(e.target.value))
          ),
        ]),
        buttonSet(dispatch),
      ]
